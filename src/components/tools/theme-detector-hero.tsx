...
interface ThemeDetectorHeroProps {
  defaultUrl?: string;
}

export function ThemeDetectorHero({ defaultUrl = "" }: ThemeDetectorHeroProps) {
  const [url, setUrl] = useState(defaultUrl);
  const { scan, loading, error } = useStoreScan({ resultTab: "theme" });
    ...
    <div className="mx-auto w-full max-w-5xl">
      <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-lg shadow-brand/5 sm:p-7 lg:p-8">
        <form onSubmit={handleSubmit}>
          <label htmlFor="theme-detector-url" className="sr-only">
            Paste a URL to detect Shopify theme
          </label>
      ...
      <p className="mt-4 text-center text-xs text-slate-400">
        Free Shopify Theme Detector · Detect Shopify theme on any public store ·
        Results in seconds
      </p>
    </div>
  );
